...
Bullet::draw()
{
  if (base.x >= scroll_x - base.width &&
      base.x <= scroll_x + screen->w)
    {
      img_bullet->draw((base.x - scroll_x)/32.*12., base.y/32.*12.);
    }
}
  ...
  if (base.height < 32)
    {
      ...
      dest.y = (int)((base.y + 32 - base.height)/32.*12.);
      dest.w = 12;
      dest.h = (int)(base.height/32.*12.);
  ...
  else
    {
      ...
      if (kind == UPGRADE_GROWUP)
        {
          img_growup->draw(targetX, targetY);
        }
      else if (kind == UPGRADE_ICEFLOWER)
        {
          img_iceflower->draw(targetX, targetY);
        }
      else if (kind == UPGRADE_HERRING)
        {
          img_star->draw(targetX, targetY);
        }
      else if (kind == UPGRADE_1UP)
        {
          img_1up->draw(targetX, targetY);
        }
    }
}
