...
void ParticleSystem::draw(float scrollx, float scrolly, int layer)
{
    ...
    for(i = particles.begin(); i != particles.end(); ++i) {
        ...
        float x = fmodf((particle->x - scrollx)/32.*12., virtual_width);
        if(x < 0) x += virtual_width;
        float y = fmodf((particle->y - scrolly)/32.*12., virtual_height);
        if(y < 0) y += virtual_height;
        float xmax = fmodf((x + particle->texture->w)/32.*12., virtual_width);
        float ymax = fmodf((y + particle->texture->h)/32.*12., virtual_height);
