...
void st_directory_setup(void)
{
  ...
  st_dir = (char *) malloc(sizeof(char) * (strlen(home) +
                                           strlen("/supertuxSaves") + 1));
  strcpy(st_dir, home);
  strcat(st_dir, "/supertuxSaves");
...
void st_menu(void)
{
  ...
  main_menu->set_pos(screen->w/2, screen->h/2+40);
...
void st_video_setup(void)
{
  ...
  if (SDL_Init(SDL_INIT_VIDEO) < 0)
    {
      iprintf("Error: I could not initialize video!\n"
...
{
	// this is the Nintendo DS.
	// we don't need no sdl mixer, i'm doing things MY way.
	char binfile[128];
	sprintf(binfile, "%s/soundbank.bin", datadir.c_str());
	iprintf("%s\n", binfile);

	mp3_init();
	mmInitDefault(binfile);
	open_audio();
	use_sound = true;
	use_music = true;
	audio_device = true;
