                  ...
                  if (strcmp(lisp_symbol(lisp_car(element)), "level") == 0)
                    {
                      ...
                      reader.read_bool("apply-action-right", &level.apply_action_east);
						...
						// don't get the title like this. it causes an assertion error
						// from reading a shit-ton of level files just to get the title.
						// cram them into the worldmap file instead, it loads faster too.
...
WorldMap::get_input()
{
  ...
  input_direction = D_NONE;
	...
	else
	{
		...
		else if (keyH & KEY_RIGHT)
			input_direction = D_EAST;
		else if (keyH & KEY_UP)
			input_direction = D_NORTH;
		else if (keyH & KEY_DOWN)
    ...
    for(int x = 0; x < width; ++x)
      {
        ...
        tile->sprite->draw(x*12 + offset.x,
                           y*12 + offset.y);
      }
  ...
  for(Levels::iterator i = levels.begin(); i != levels.end(); ++i)
    {
      if(i->name.empty()) {
      	if ((i->teleport_dest_x != -1) && !i->invisible_teleporter) {
				leveldot_teleporter->draw(i->x*12 + offset.x, 
                             i->y*12 + offset.y);
			}
			else continue;
		}
        ...
        leveldot_green->draw(i->x*12 + offset.x, 
                             i->y*12 + offset.y);
      else
        leveldot_red->draw(i->x*12 + offset.x, 
                           i->y*12 + offset.y);        
    }
  ...
  while(!quit)
    {
      ...
      if (1)
        {
          offset.x = (-tux_pos.x/32.*12.) + screen->w/2;
          offset.y = (-tux_pos.y/32.*12.) + screen->h/2;

          if (offset.x > 0) offset.x = 0;
          if (offset.y > 0) offset.y = 0;

          if (offset.x < screen->w - width*12) offset.x = screen->w - width*12;
          if (offset.y < screen->h - height*12) offset.y = screen->h - height*12;
        } 
