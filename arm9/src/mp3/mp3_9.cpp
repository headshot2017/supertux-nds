...
int mp3_init() {return 0;}
bool mp3_is_playing() {return false;}
int mp3_set_volume(int volume) {return 0;}
int mp3_resume() {return 0;}
int mp3_pause() {return 0;}
int mp3_stop() {return 0;}
int mp3_play(const char *filename, int loop, float loopsec) {return 0;}
void mp3_fill_buffer() {}
...
volatile wav_stuffs* wav_load_handle(const char *filename, int* error_output)
{
	...
	if (!drwav_init_file(&wavfp, filename, NULL)) {
		if (error_output) *error_output = 1;
		return 0;
	}
	...
	if (pSampleData == 0)
	{
		drwav_uninit(&wavfp);
		free((void*)wav2);
		if (error_output) *error_output = 2;
		return 0;
	}
	if (drwav_read_pcm_frames(&wavfp, wavfp.totalPCMFrameCount, pSampleData) == 0)
	{
		...
		free((void*)wav2);
		free(pSampleData);
		if (error_output) *error_output = 3;
		return 0;
	}
	...
	if (wavfp.bitsPerSample == 8) // 8 bit
	{
		...
		drwav_u8_to_s16(_8bitdata, pSampleData, wavfp.totalPCMFrameCount);
		free(pSampleData);
		...
		*/
		...
		free((void*)wav2);
		free(pSampleData);
		if (error_output) *error_output = 4;
		return 0;
	}
	...
	if (result == 0) // no slots
	{
		...
		free((void*)wav2);
		free(pSampleData);
		if (error_output) *error_output = 5;
		return 0;
	}

	if (error_output) *error_output = 0;
	return wav2;
}
