...
void draw_demo(GameSession* session, double frame_ratio)
{
  ...
  tux->key_event(keymap.right,DOWN);
  ...
  if(random_timer.check())
    {
      if(walking)
        ...
        tux->key_event(keymap.jump,DOWN);
    }
  else
    {
...
{
	consoleClear();
	iprintf("\x1b[39;0m SuperTux " VERSION "\n"
			"Copyright (c) 2003\n"
			"SuperTux Devel Team\n\n"
...
void title(void)
{
  ...
  titleSession = new GameSession(datadir + "/levels/misc/menu.stl", 0, ST_GL_DEMO_GAME);
  ...
  while (Menu::current())
    {
      ...
      /* Draw the background: */
      draw_demo(titleSession, frame_ratio);
      
      if (Menu::current() == main_menu)
        logo->draw( 160/2.5, 30/2.5);

		// copyright message moved to bottom screen
