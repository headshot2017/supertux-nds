...
BadGuy::action(double frame_ratio)
{
  ...
  static int seentimes = 0;
  if (base.x <= scroll_x + 640 + OFFSCREEN_DISTANCE && !seen)
  {
    seen = true;
	seentimes++;
	printf("seen! %d\n", seentimes);
...
BadGuy::draw()
{
  ...
  float to12 = 1/32.*12.;
  if(base.x*to12 <= (scroll_x - base.width)*to12 || base.x*to12 >= scroll_x*to12 + screen->w)
    return;
  ...
  sprite->draw((base.x - scroll_x)*to12, base.y*to12);
