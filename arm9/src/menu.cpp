                ...
                int menu_height)
{
  ...
  int x_pos       = pos_x;
  int y_pos       = pos_y + 12*index - menu_height/2 + 8 + effect_offset;
  int shadow_size = 2;
...
Menu::draw()
{
  ...
  fillrect(pos_x - menu_width/2,
           pos_y - 12*item.size()/2 - 4,
           menu_width,menu_height + 16,
           150,180,200,125);
...
void Menu::event()
{
	scanKeys(); // todo: move this to a separate file, like dsinput.cpp
	u32 key = keysDown();
