...
int Player::key_event(u32 key, int state)
{
	bool did = false;
...
void Player::DSkeys(u32 key)
{
	...
	//if (!(key & KEY_Y))
		//input.old_fire = UP;
	input.fire = !!(key & KEY_Y);
	//iprintf("%d %d %d\n", input.old_fire, input.fire, key & KEY_Y);
}
...
Player::draw()
{
	float targetX = (base.x - scroll_x) / 32. * 12.;
	float targetY = base.y / 32. * 12.;
	
  if (!safe_timer.started() || (global_frame_counter % 2) == 0)
    {
      if (dying == DYING_SQUISHED)
        {
          smalltux_gameover->draw(targetX, targetY);
        }
      else
        {
          PlayerSprite* sprite;
          ...
          if (duck && size != SMALL)
            {
              if (dir == RIGHT)
                sprite->duck_right->draw(targetX, targetY);
              else 
                sprite->duck_left->draw(targetX, targetY);
            }
          else if (skidding_timer.started())
            {
              if (dir == RIGHT)
                sprite->skid_right->draw(targetX, targetY);
              else
                sprite->skid_left->draw(targetX, targetY); 
            }
          else if (kick_timer.started())
            {
              if (dir == RIGHT)
                sprite->kick_right->draw(targetX, targetY);
              else
                sprite->kick_left->draw(targetX, targetY); 
            }
          else if (physic.get_velocity_y() != 0)
            {
              if (dir == RIGHT)
                sprite->jump_right->draw(targetX, targetY);
              else
                sprite->jump_left->draw(targetX, targetY);                   
            }
          else
            {
              if (fabsf(physic.get_velocity_x()) < 1.0f) // standing
                {
                  if (dir == RIGHT)
                    sprite->stand_right->draw( targetX, targetY);
                  else
                    sprite->stand_left->draw( targetX, targetY);
                }
              else // moving
                {
                  if (dir == RIGHT)
                    sprite->walk_right->draw(targetX, targetY);
                  else
                    sprite->walk_left->draw(targetX, targetY);
                }
            }
          ...
          if (holding_something && physic.get_velocity_y() == 0 && !duck)
            {
              if (dir == RIGHT)
                sprite->grab_right->draw(targetX, targetY);
              else
                sprite->grab_left->draw(targetX, targetY);
            }
          ...
          if (invincible_timer.started() &&
            ...
            {
              if (size == SMALL || duck)
                smalltux_star->draw(targetX, targetY);
              else
                largetux_star->draw(targetX, targetY);
            }
        }
    }     
