...
GameSession::levelintro(void)
{
  ...
  gold_text->drawf(str, 0, screen->h/2-8, A_HMIDDLE, A_TOP, 1);

  sprintf(str, "TUX x %d", player_status.lives);
  white_text->drawf(str, 0, screen->h/2, A_HMIDDLE, A_TOP, 1);
  
  sprintf(str, "by %s", world->get_level()->author.c_str());
  white_small_text->drawf(str, 0, screen->h/2+64, A_HMIDDLE, A_TOP, 1);
  ...
  if (end_sequence != NO_ENDSEQUENCE)
	...
	else // normal mode
	{
		...
		if (Menu::current())
		{
			...
			Player& tux = *world->get_tux();
			tux.key_event(keymap.jump, UP);
			tux.key_event(keymap.duck, UP);
			tux.key_event(keymap.left, UP);
			tux.key_event(keymap.right, UP);
			tux.key_event(keymap.fire, UP);
		}
		else
		{
			Player& tux = *world->get_tux();
  ...
  if(game_pause)
    {
      ...
      blue_text->drawf("PAUSE - Press 'P' To Play", 0, screen->h/2-4, A_HMIDDLE, A_TOP, 1);
    }
...
GameSession::run()
{
  ...
  consoleClear();
	// \x1b[39;0m reset color
	// \x1b[33;1m yellow
	// \x1b[%d;%dH print coordinates (y,x)
	int coinsX = 31;
	int livesX = 30;
	int temp = player_status.distros;
	while ((temp /= 10)) coinsX--;
	temp = player_status.lives;
	while ((temp /= 10)) livesX--;
  iprintf("SCORE \x1b[33;1m%d\x1b[39;0m", player_status.score);
  iprintf("\x1b[0;12HTIME \x1b[33;1m%d\x1b[39;0m", world->get_level()->time_left-1);
  iprintf("\x1b[0;24HCOINS\x1b[0;%dH\x1b[33;1m%d\x1b[39;0m", coinsX, player_status.distros);
  iprintf("\x1b[1;24HLIVES\x1b[1;%dH\x1b[33;1m%dx\x1b[39;0m", livesX, player_status.lives);
  iprintf("\x1b[3;0H"); // move cursor somewhere for printing text
...
{
	// handle bottom screen HUD
	bool change = false;
	static int lastScore = player_status.score;
	static int lastDistros = player_status.distros;
	static int lastLives = player_status.lives;
	static int lastTime = time_left.get_left() / 1000;

	if (lastScore != player_status.score)
	{
		change = true;
		lastScore = player_status.score;
		iprintf("\x1b[0;0HSCORE \x1b[33;1m%d\x1b[39;0m", player_status.score);
	}
	if (lastDistros != player_status.distros)
	{
		change = true;
		lastDistros = player_status.distros;
		int temp = player_status.distros;
		int coinsX = 31;
		while ((temp /= 10)) coinsX--;
		iprintf("\x1b[0;24HCOINS\x1b[0;%dH\x1b[33;1m%d\x1b[39;0m", coinsX, player_status.distros);
	}
	if (lastLives != player_status.lives)
	{
		change = true;
		lastLives = player_status.lives;
		int temp = player_status.lives;
		int livesX = 30;
		while ((temp /= 10)) livesX--;
		iprintf("\x1b[1;24HLIVES\x1b[1;%dH\x1b[33;1m%dx\x1b[39;0m", livesX, player_status.lives);
	}
	if (lastTime != time_left.get_left() / 1000)
	{
		change = true;
		lastTime = time_left.get_left() / 1000;
		iprintf("\x1b[0;12HTIME \x1b[33;1m%d\x1b[39;0m", lastTime);
	}

	if (change) iprintf("\x1b[3;0H"); // move cursor somewhere for printing text
